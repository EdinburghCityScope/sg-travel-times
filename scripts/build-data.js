...
const csvFilename = 'travel-times.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?destination ?method ?mean
            WHERE {
                ?s qb:dataSet data:travel-times ;
                   qb:measureType prop:mean ;
                   prop:mean ?mean ;
                   dim:methodOfTravel ?m ;
                   dim:destination ?d ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
                ?d rdfs:label ?destination .
                ?z rdfs:label ?zone .
                ?m rdfs:label ?method .
                ?y rdfs:label ?year .
                FILTER ( ?z IN (
